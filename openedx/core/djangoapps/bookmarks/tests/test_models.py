...
class BookmarksTestsBase(ModuleStoreTestCase):
    ...
    def setup_data(self, store_type=ModuleStoreEnum.Type.mongo):
        ...
        self.other_bookmark_1 = BookmarkFactory.create(
            user=self.user,
            course_key=six.text_type(self.other_course.id),
                    ...
                    for block in blocks_at_current_level:
                        for __ in range(children_per_block):
                            blocks_at_next_level += [ItemFactory.create(
                                parent_location=block.scope_ids.usage_id, display_name=six.text_type(display_name)
                            )]
                            display_name += 1
        ...
        with self.store.default_store(store_type):
            ...
            with self.store.bulk_operations(course.id):
                blocks = [ItemFactory.create(
                    parent_location=course.location, category='chapter', display_name=six.text_type(index)
    ...
    def assert_bookmark_model_is_valid(self, bookmark, bookmark_data):
        ...
        self.assertEqual(six.text_type(bookmark.usage_key), six.text_type(bookmark_data['usage_key']))
    ...
    def assert_bookmark_data_is_valid(self, bookmark, bookmark_data, check_optional_fields=False):
        ...
        self.assertEqual(bookmark_data['course_id'], six.text_type(bookmark.course_key))
        self.assertEqual(bookmark_data['usage_id'], six.text_type(bookmark.usage_key))
        self.assertEqual(bookmark_data['block_type'], six.text_type(bookmark.usage_key.block_type))
...
class XBlockCacheModelTest(ModuleStoreTestCase):
    ...
    PATH1 = [
        [six.text_type(CHAPTER1_USAGE_KEY), 'Chapter 1'],
        [six.text_type(SECTION1_USAGE_KEY), 'Section 1'],
    ]
    PATH2 = [
        [six.text_type(CHAPTER1_USAGE_KEY), 'Chapter 1'],
        [six.text_type(SECTION2_USAGE_KEY), 'Section 2'],
    ]
